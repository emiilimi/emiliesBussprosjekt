...
function gjennomsnittligForsinkelse(data) {
	...
	const snittForsinket =
		utenNegative.reduce((sum, value) => sum + value, 0) / utenNegative.length;
...
async function fetchData() {
	try {
		...
		let a,
			b,
			c,
			d = gjennomsnittligForsinkelse(data);
		console.log(a);
		console.log(b);
		console.log(c);
		console.log(d);
...
function generateTable(data) {
	...
	const meanDelay =
		diffDates.reduce((sum, date) => sum + date, 0) / diffDates.length;
